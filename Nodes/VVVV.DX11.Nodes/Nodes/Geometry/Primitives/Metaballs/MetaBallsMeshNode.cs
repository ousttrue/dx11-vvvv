...
namespace VVVV.Nodes
{
    ...
    public class MetaballsMeshNode : IPluginEvaluate, IDisposable, IDX11ResourceProvider
    {
        ...
        {
            if (this.FGeom[0] == null) { this.FGeom[0] = new DX11Resource<DX11IndexedGeometry>(); }

            if (pos.SliceCount != NumBalls || m_Ball == null) //initialize metaball arrays
            {
            ...
            if (update) //initialize all tables
            {
                ...
                IxBuffer = new int[NumIndices];

                unsafe
                {
                    fixed (int* IxBufferPtr = &IxBuffer[0])
                    {
                        fixed (int* IndicesPtr = &Indices[0])
                        {
                            for (int j = 0; j < NumIndices; j++)
                                *(IxBufferPtr + j) = (int)*(IndicesPtr + j);
                        }
                    }
                }
