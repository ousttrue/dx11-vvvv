...
{
    [PluginInfo(Name = "Writer", Category = "DX11.Texture", Version = "1d", Author = "vux", AutoEvaluate = true)]
    public class WriterTexture1dNode : IPluginEvaluate, IDX11ResourceDataRetriever
    {
        [Input("Texture In")]
                ...
                for (int i = 0; i < SpreadMax; i++)
                {
                    if (this.FTextureIn[i].Contains(context) && this.FInSave[i])
                    {
                        ...
                        try
                        {
                            ...
                            Texture1D.SaveTextureToFile(this.AssignedContext.Device.ImmediateContext, this.FTextureIn[i][context].Resource, ImageFileFormat.Dds, path);
